...
def configure_logging(level: str = "INFO", *, json_output: bool = True) -> None:
    """Configure the ``emrag`` logger tree.
    ...
    """
    logger = logging.getLogger("emrag")
...
def get_logger(name: str) -> logging.Logger:
    """Return a logger namespaced under ``emrag``."""
    return logging.getLogger(name if name.startswith("emrag") else f"emrag.{name}")
